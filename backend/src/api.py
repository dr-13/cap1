...
def post_drinks(jwt):
    ...
    new_drink = Drink(title=title, recipe=json.dumps(revipe))
    new_drink.insert()
...
def edit_drink(jwt, id):
    ...
    current_drink.recipe = recipe
    current_drink.update()

    return jsonify({'success': True, 'drinks': [current_drink.long()]}, 200)
...
def not_found(error):
    return jsonify({'success': False, 'error': 404, 'message': 'Cannot find what you want'})
...
def not_found(error):
    return jsonify({'success': False, 'error': 401, 'message': 'Authorization failed'})
